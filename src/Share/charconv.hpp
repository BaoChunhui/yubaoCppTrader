 ...
 * \date 2026/03/11
 * 
 * \brief 字符编码转换的封装，实现GB2312和UTF-8之间的转换
 */
#pragma once
#include <stdlib.h>
#include <string>
...
#else
			iconv_t cd;
			t_string = new char[dst_len + 1];
			char* p = t_string;
			cd = iconv_open("gb2312", "utf-8");
			if (cd != (iconv_t)-1)
			{
				memset(t_string, 0, dst_len + 1);
...
#else
			iconv_t cd;
			utf8_string = new char[dst_len + 1];
			char* p = utf8_string;
			cd = iconv_open("utf-8", "gb2312");
			if (cd != (iconv_t)-1)
			{
				memset(utf8_string, 0, dst_len + 1);
...
public:
	...
	{ 
		...
		{  
			const char cc = src[i];  
			if ((static_cast<unsigned char>(cc) & 0x80) == 0)
			{  
				if (cc == ' ')  
				{  
					encoded_string += "%20";  
