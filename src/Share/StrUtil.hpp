...
{
public:
    // 修改传入的字符串，移除首尾指定分隔符（默认空格、制表符、回车）
    static inline void trim(std::string& str, const char* delims = " \t\r", bool left = true, bool right = true)
    {
        ...
        if (right)
        {
            const auto end = str.find_last_not_of(delims);
        ...
        if (left)
        {
            const auto begin = str.find_first_not_of(delims);
    ...
    // 将分割结果写入传入的StringVector（避免返回值拷贝）
	...
	*/
	static inline std::string standardisePath( const std::string &init, bool bIsDir = true)
	{
		...
		if (bIsDir && !path.empty() && path[path.length() - 1] != '/')
			path += '/';
	...
	*/
	...
	// 为std::string添加类似于CString的Format这个函数的
	...
	static inline std::string printf2(const char *pszFormat,va_list argptr)
	{
		...
		while (1)
		{
			...
			// C89 doesn't define va_copy; most platforms still provide it.
#endif
			va_list argcopy;
			va_copy(argcopy, argptr);
#ifdef _MSC_VER
			int n = _vsnprintf(buffer, size, pszFormat, argcopy);
#else
			int n = vsnprintf(buffer, size, pszFormat, argcopy);
#endif
			va_end(argcopy);
