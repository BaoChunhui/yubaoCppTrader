...
public:
	...
	static inline int32_t getTZOffset()
	{
		...
		if (offset == 99)
		{
			...
			tm tm_ltm, tm_gtm, _gtm2;
#ifdef _MSC_VER
			localtime_s(&tm_ltm, &now);
			gmtime_s(&tm_gtm, &now);
#else
			localtime_r(&now, &tm_ltm);
			gmtime_r(&now, &tm_gtm);
#endif
...
#endif
			int64_t secDiff = (int64_t)(now - _gt) + (_gtm2.tm_isdst ? 3600 : 0);
			offset = (int32_t)(secDiff / 3600);
		}
		return offset;
	}
    ...
    static uint32_t getNextMonth(uint32_t curMonth, int months = 1)
    {
        ...
        uYear += uAddYear;
        uMonth += uAddMon;
        while (uMonth > 12) { uMonth -= 12; ++uYear; }
        while (uMonth < 1)  { uMonth += 12; --uYear; }
	...
	{
	public:
		Time32() : _msec(0) { memset(&t, 0, sizeof(tm)); }
