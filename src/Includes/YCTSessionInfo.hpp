/*!
 * \file YCTSessionInfo.hpp
 * \project	yubaoCppTrader
 *
 * \author yubao
 * \date 2026/03/30
 * 
 * \brief yubaoCppTrader交易时间模板对象定义
 */
#pragma once
#include <vector>
...
class YCTSessionInfo : public YCTObject
{
public:
	//交易时段
...
protected:
	...
	YCTSessionInfo(int32_t offset)
	{
		m_uOffsetMins = offset;
	}
	virtual ~YCTSessionInfo(){}

public:
	const char* id() const{ return m_strID.c_str(); }
	const char* name() const{ return m_strName.c_str(); }

	static YCTSessionInfo* create(const char* sid, const char* name, int32_t offset = 0)
	{
		YCTSessionInfo* pRet = new YCTSessionInfo(offset);
		pRet->m_strID = sid;
		pRet->m_strName = name;
		return pRet;
	}
