/*!
 * \file YCTContractInfo.hpp
 * \project	yubaoCppTrader
 *
 * \author yubao
 * \date 2026/03/29
 * 
 * \brief yubaoCppTrader合约信息定义文件
 */
#pragma once
#include "YCTMacros.h"
...
class YCTCommodityInfo : public YCTObject
{
private:
    ...
    double              m_dPriceTick;        //价格最小变动单位

    double              m_dLotTick;          //数量精度
    double              m_dMinLots;          //最小交易量

    ContractCategory    m_ccCategory;        //品种分类，期货、股票、期权等
    CoverMode           m_coverMode;         //平仓模式，开平、开平昨平今等
    PriceMode           m_priceMode;         //价格模式
    TradingMode         m_tradeMode;         //交易模式

    CodeSet             m_setCodes;

    YCTSessionInfo*     m_pSession;          //交易时段信息

    double              m_dOpenFee;          //开仓手续费
    double              m_dCloseFee;         //平仓手续费
    double              m_dCloseTFee;        //平今手续费
    int                 m_nFeeAlg;           //手续费算法，-1表示不计算，0按成交量计算，1按成交额计算
    double              m_dMarginRate;       //保证金比例

protected:
    YCTCommodityInfo()
        : m_uVolScale(0)
        , m_dPriceTick(0), m_dLotTick(0), m_dMinLots(0)
        , m_ccCategory(ContractCategory::CC_Stock)
        , m_coverMode(CoverMode::CM_OpenCover)
        , m_priceMode(PriceMode::PM_Both)
        , m_tradeMode(TradingMode::TM_Both)
        , m_pSession(NULL)
        , m_dOpenFee(0), m_dCloseFee(0), m_dCloseTFee(0)
        , m_nFeeAlg(-1)
        , m_dMarginRate(0) {}
    virtual ~YCTCommodityInfo() {}

public:
    static YCTCommodityInfo* create(const char* product_id, const char* name, const char* exchg, const char* session, const char* trdtpl, const char* currency = "CNY")
    {
        YCTCommodityInfo* ret = new YCTCommodityInfo();
        ret->m_strName = name;
        ret->m_strExchg = exchg;
        ret->m_strProduct = product_id;
    ...
    inline void setPriceTick(double priceTick) { m_dPriceTick = priceTick; }
    inline void setCategory(ContractCategory category) { m_ccCategory = category; }
    inline void setCoverMode(CoverMode coverMode) { m_coverMode = coverMode; }
    inline void setPriceMode(PriceMode priceMode) { m_priceMode = priceMode; }
    inline void setTradingMode(TradingMode tradingMode) { m_tradeMode = tradingMode; }

    inline bool canShort() const { return m_tradeMode == TradingMode::TM_Both; }
    inline bool isT1() const { return m_tradeMode == TradingMode::TM_LongT1; }
	...
	inline ContractCategory		getCategory() const{ return m_ccCategory; }
	...
	{
		return (m_ccCategory == ContractCategory::CC_FutOption || m_ccCategory == ContractCategory::CC_ETFOption || m_ccCategory == ContractCategory::CC_SpotOption);
	}

	inline bool isFuture() const
	{
		return m_ccCategory == ContractCategory::CC_Future;
	}

	inline bool isStock() const
	{
		return m_ccCategory == ContractCategory::CC_Stock;
	}

	inline double	getLotsTick() const { return m_dLotTick; }
	inline double	getMinLots() const { return m_dMinLots; }

	inline void		setSessionInfo(YCTSessionInfo* sInfo) { m_pSession = sInfo; }
	inline YCTSessionInfo* getSessionInfo() const { return m_pSession; }
...
class YCTContractInfo :	public YCTObject
{
private:
...
protected:
    YCTContractInfo()
        : m_maxMktQty(0), m_maxLmtQty(0), m_minMktQty(0), m_minLmtQty(0)
        , m_openDate(19900101), m_expireDate(30991231)
        , m_lMarginRatio(0), m_sMarginRatio(0), m_uMarginFlag(0)
        , m_dOpenFee(0), m_dCloseFee(0), m_dCloseTFee(0)
        , m_nFeeAlg(-1)
        , m_commInfo(NULL)
        , m_uHotFlag(0)
        , m_uTotalIdx(INVALID_UINT32)
        , m_pExtData(NULL) {}
    virtual ~YCTContractInfo() {}

public:
    static YCTContractInfo* create(const char* code, const char* name, const char* exchg, const char* pid)
    {
        YCTContractInfo* ret = new YCTContractInfo;
        ret->m_strCode = code;
        ret->m_strName = name;
    ...
    inline double	getLongMarginRatio() const {
        ...
        return commRate == 0.0 ? m_lMarginRatio : commRate;
    }
    ...
    inline double	getShortMarginRatio() const {
        ...
        double commRate = m_commInfo->getMarginRate();
        return commRate == 0.0 ? m_sMarginRatio : commRate;
    }

    inline void setCommInfo(YCTCommodityInfo* commInfo) { m_commInfo = commInfo; }
    inline YCTCommodityInfo* getCommInfo() const { return m_commInfo; }
