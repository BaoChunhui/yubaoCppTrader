 ...
 * \date 2026/03/14
 * 
 * \brief yubaoCppTrader行情数据定义文件,包括tick、bar、orderqueue、orderdetail、transaction等数据
 */
#pragma once
#include <stdlib.h>
#include <algorithm>
 ...
 *	数值的数据类型为double
 */
class YCTValueArray : public YCTObject
{
protected:
...
public:
	 ...
	 *	@decimal 保留的小数点位数
	 */
    static YCTValueArray* create()
    {
        YCTValueArray* pRet = new YCTValueArray;
	...
	{
		if (idx < 0)
			return static_cast<int32_t>(m_vecData.size()) + idx;
		return idx;
	}

	/*
	 *	读取指定位置的数据，支持负索引（-1 表示最后一个）
	 *	如果超出范围,则返回 INVALID_DOUBLE
	 */
	inline double at(int32_t idx) const
	{
		int32_t pos = translateIdx(idx);
		if (pos < 0 || static_cast<uint32_t>(pos) >= m_vecData.size())
			return INVALID_DOUBLE;
		return m_vecData[static_cast<uint32_t>(pos)];
	}
	...
	double maxvalue(int32_t head, int32_t tail, bool isAbs = false) const
	{
		...
		if (head < 0 || tail < 0 ||
		    static_cast<uint32_t>(head) >= m_vecData.size() ||
		    static_cast<uint32_t>(tail) >= m_vecData.size())
			return INVALID_DOUBLE;

		uint32_t begin = static_cast<uint32_t>(std::min(head, tail));
		uint32_t end = static_cast<uint32_t>(std::max(head, tail));
	...
	/*
	 *	设置指定位置的数据，支持负索引（-1 表示最后一个）
	 */
	inline void set(int32_t idx, double val)
	{
		int32_t pos = translateIdx(idx);
		if (pos < 0 || static_cast<uint32_t>(pos) >= m_vecData.size())
			return;
		m_vecData[static_cast<uint32_t>(pos)] = val;
	}
 ...
 *	所以有两个开始地址
 */
class YCTKlineSlice : public YCTObject
{
private:
	char _code[MAX_INSTRUMENT_LENGTH];
	YCTKlinePeriod _period;
	uint32_t _times;
	typedef std::pair<YCTBarStruct*, uint32_t> BarBlock;
	std::vector<BarBlock> _blocks;
	uint32_t _count;
protected:
	YCTKlineSlice()
		: _period(YCTKlinePeriod::KP_Minute1)
		, _times(1)
		, _count(0)
	{
...
public:
	static YCTKlineSlice* create(const char* code, YCTKlinePeriod period, uint32_t times, YCTBarStruct* bars = NULL, int32_t count = 0)
	{
		if (code == NULL)
			return NULL;
		...
		YCTKlineSlice* pRet = new YCTKlineSlice;
		if (count < 0)
			count = 0;
		if (bars == NULL)
			count = 0;
		memcpy(pRet->_code, code, len);
		pRet->_code[len] = 0;
		pRet->_period = period;
		pRet->_times = times;
		if (bars)
	...
	inline bool appendBlock(YCTBarStruct* bars, uint32_t count)
	{
	...
	inline YCTBarStruct* get_block_addr(std::size_t blkIdx)
	{
	...
	inline YCTBarStruct* at(int32_t idx)
	{
		if (_count == 0)
			return NULL;
	...
	inline const YCTBarStruct* at(int32_t idx) const
	{
		if (_count == 0)
			return NULL;
	...
	*/
	double maxprice(int32_t head, int32_t tail) const
	{
		...
		int32_t last = size() - 1;
		int32_t begin = std::max(0, std::min(std::min(head, tail), last));
		int32_t end = std::min(std::max(head, tail), last);
		if (begin > end)
			return INVALID_DOUBLE;
	...
	*/
	double minprice(int32_t head, int32_t tail) const
	{
		...
		double minValue = this->at(begin)->low;
		for (int32_t i = begin; i <= end; i++)
		{
	...
	*/
	YCTValueArray* extractData(YCTKlineFieldType type, int32_t head = 0, int32_t tail = -1) const
	{
		if (_count == 0)
			return NULL;
		...
		YCTValueArray* vArray = YCTValueArray::create();

		for (int32_t i = begin; i <= end; i++)
		{
			const YCTBarStruct& day = *at(i);
			switch (type)
			{
			case YCTKlineFieldType::KFT_OPEN:
				vArray->append(day.open);
				break;
			case YCTKlineFieldType::KFT_HIGH:
				vArray->append(day.high);
				break;
			case YCTKlineFieldType::KFT_LOW:
				vArray->append(day.low);
				break;
			case YCTKlineFieldType::KFT_CLOSE:
				vArray->append(day.close);
				break;
			case YCTKlineFieldType::KFT_VOLUME:
				vArray->append(day.vol);
				break;
			case YCTKlineFieldType::KFT_SVOLUME:
				if (day.vol > INT_MAX)
					vArray->append(1 * ((day.close > day.open) ? 1 : -1));
				else
					vArray->append((int32_t)day.vol * ((day.close > day.open) ? 1 : -1));
				break;
			case YCTKlineFieldType::KFT_DATE:
				vArray->append(day.date);
				break;
			case YCTKlineFieldType::KFT_TIME:
...
/*
 *	K线数据
 *	K线数据的内部数据使用YCTBarStruct
 *	YCTBarStruct是一个结构体
 *	因为K线数据单独使用的可能性较低
 *	所以不做YCTObject派生类的封装
 */
class YCTKlineData : public YCTObject
{
public:
	typedef std::vector<YCTBarStruct> YCTBarList;

protected:
	char			m_strCode[MAX_INSTRUMENT_LENGTH];
	YCTKlinePeriod	m_kpPeriod;
	uint32_t		m_uTimes;
	bool			m_bUnixTime;	//是否是时间戳格式,目前只在秒线上有效
	YCTBarList		m_vecBarData;
	bool			m_bClosed;		//是否是闭合K线

	YCTKlineData()
		: m_kpPeriod(YCTKlinePeriod::KP_Minute1)
		, m_uTimes(1)
		, m_bUnixTime(false)
		, m_bClosed(true)
	{
...
public:
	 ...
	 *	@size 初始分配的数据长度
	 */
	static YCTKlineData* create(const char* code, uint32_t size)
	{
		if (code == NULL)
			return NULL;
		...
		YCTKlineData* pRet = new YCTKlineData;
	...
	inline void setPeriod(YCTKlinePeriod period, uint32_t times = 1){ m_kpPeriod = period; m_uTimes = times; }
	inline void setUnixTime(bool bEnabled = true){ m_bUnixTime = bEnabled; }
	
	inline YCTKlinePeriod period() const{ return m_kpPeriod; }
	...
	YCTValueArray* extractData(YCTKlineFieldType type, int32_t head = 0, int32_t tail = -1) const
	{
		...
		YCTValueArray *vArray = NULL;

		vArray = YCTValueArray::create();

		for(uint32_t i = begin; i <= end; i++)
		{
			const YCTBarStruct& day = m_vecBarData.at(i);
			switch(type)
			{
			case YCTKlineFieldType::KFT_OPEN:
				vArray->append(day.open);
	 ...
	 *	获取K线内部vector的引用
	 */
	inline YCTBarList& getDataRef(){ return m_vecBarData; }
	 ...
	 *	追加一条K线
	 */
	inline void	appendBar(const YCTBarStruct& bar)
	{
		...
		{
			YCTBarStruct* lastBar = at(-1);
			if(lastBar->date==bar.date && lastBar->time==bar.time)
			{
				memcpy(lastBar, &bar, sizeof(YCTBarStruct));
			}
			else
			{
				m_vecBarData.emplace_back(bar);
 ...
 *	封装的主要目的是出于跨语言的考虑
 */
class YCTTickData : public YCTPoolObject<YCTTickData>
{
private:
	YCTTickStruct		m_tickStruct;
	YCTContractInfo*	m_pContract;

public:
	YCTTickData() :m_pContract(NULL) {}

	/*
	 *	创建一个tick数据对象
	 *	@stdCode 合约代码
	 */
	static inline YCTTickData* create(const char* stdCode)
	{
		if (stdCode == NULL)
			return NULL;
		...
		if (len >= MAX_INSTRUMENT_LENGTH)
			return NULL;
		YCTTickData* pRet = YCTTickData::allocate();
	 ...
	 *	@tickData tick结构体
	 */
	static inline YCTTickData* create(YCTTickStruct& tickData)
	{
		YCTTickData* pRet = YCTTickData::allocate();
		memcpy(&pRet->m_tickStruct, &tickData, sizeof(YCTTickStruct));
	 ...
	 *	返回tick结构体的引用
	 */
	inline YCTTickStruct& getTickStruct(){ return m_tickStruct; }

	inline void setContractInfo(YCTContractInfo* cInfo) { m_pContract = cInfo; }
	inline YCTContractInfo* getContractInfo() const { return m_pContract; }
};

class YCTOrdQueData : public YCTObject
{
private:
	YCTOrdQueStruct	m_oqStruct;
	YCTContractInfo* m_pContract;

public:
	YCTOrdQueData() : m_pContract(NULL) {}

	static inline YCTOrdQueData* create(const char* code)
	{
		if (code == NULL)
			return NULL;
		auto len = strlen(code);
		...
		YCTOrdQueData* pRet = new YCTOrdQueData;
		memcpy(pRet->m_oqStruct.code, code, len);
		pRet->m_oqStruct.code[len] = 0;
		pRet->m_pContract = NULL;
		return pRet;
	}

	static inline YCTOrdQueData* create(YCTOrdQueStruct& ordQueData)
	{
		YCTOrdQueData* pRet = new YCTOrdQueData;
		memcpy(&pRet->m_oqStruct, &ordQueData, sizeof(YCTOrdQueStruct));
		pRet->m_pContract = NULL;
		return pRet;
	}

	inline YCTOrdQueStruct& getOrdQueStruct(){return m_oqStruct;}
	...
	inline void	setCode(const char* code) {
		...
		if (len >= MAX_INSTRUMENT_LENGTH)
			return;
		memcpy(m_oqStruct.code, code, len);
		m_oqStruct.code[len] = 0;
	}
...
class YCTOrdDtlData : public YCTObject
{
private:
	YCTOrdDtlStruct	m_odStruct;
	YCTContractInfo* m_pContract;

public:
	YCTOrdDtlData() : m_pContract(NULL) {}

	static inline YCTOrdDtlData* create(const char* code)
	{
		if (code == NULL)
			return NULL;
		auto len = strlen(code);
		...
		YCTOrdDtlData* pRet = new YCTOrdDtlData;
		memcpy(pRet->m_odStruct.code, code, len);
		pRet->m_odStruct.code[len] = 0;
		pRet->m_pContract = NULL;
		return pRet;
	}

	static inline YCTOrdDtlData* create(YCTOrdDtlStruct& odData)
	{
		YCTOrdDtlData* pRet = new YCTOrdDtlData;
		memcpy(&pRet->m_odStruct, &odData, sizeof(YCTOrdDtlStruct));
		pRet->m_pContract = NULL;
		return pRet;
	}

	inline YCTOrdDtlStruct& getOrdDtlStruct(){ return m_odStruct; }
	...
	inline void	setCode(const char* code) {
		...
		if (len >= MAX_INSTRUMENT_LENGTH)
			return;
		memcpy(m_odStruct.code, code, len);
		m_odStruct.code[len] = 0;
	}
...
class YCTTransData : public YCTObject
{
private:
	YCTTransStruct m_tsStruct;
	YCTContractInfo* m_pContract;

public:
	YCTTransData() : m_pContract(NULL) {}

	static inline YCTTransData* create(const char* code)
	{
		if (code == NULL)
			return NULL;
		auto len = strlen(code);
		...
		YCTTransData* pRet = new YCTTransData;
		memcpy(pRet->m_tsStruct.code, code, len);
		pRet->m_tsStruct.code[len] = 0;
		pRet->m_pContract = NULL;
		return pRet;
	}

	static inline YCTTransData* create(YCTTransStruct& transData)
	{
		YCTTransData* pRet = new YCTTransData;
		memcpy(&pRet->m_tsStruct, &transData, sizeof(YCTTransStruct));
		pRet->m_pContract = NULL;
		return pRet;
	}
	...
	inline YCTTransStruct& getTransStruct(){ return m_tsStruct; }
	...
	inline void	setCode(const char* code) {
		...
		if (len >= MAX_INSTRUMENT_LENGTH)
			return;
		memcpy(m_tsStruct.code, code, len);
		m_tsStruct.code[len] = 0;
	}
 ...
 */
class YCTHisTickData : public YCTObject
{
protected:
	char						m_strCode[32];
	std::vector<YCTTickStruct>	m_ayTicks;
	bool						m_bValidOnly;
	double						m_dFactor;

	YCTHisTickData() :m_bValidOnly(false), m_dFactor(1.0){}
...
public:
	...
	*	@param nSize 预先分配的大小
	*/
	static inline YCTHisTickData* create(const char* stdCode, unsigned int nSize = 0, bool bValidOnly = false, double factor = 1.0)
	{
		if (stdCode == NULL)
			return NULL;
		auto len = strlen(stdCode);
		...
		YCTHisTickData *pRet = new YCTHisTickData;
		memcpy(pRet->m_strCode, stdCode, len);
		pRet->m_strCode[len] = 0;
	...
	*/
	static inline YCTHisTickData* create(const char* stdCode, bool bValidOnly = false, double factor = 1.0)
	{
		if (stdCode == NULL)
			return NULL;
		auto len = strlen(stdCode);
	...
	*	
	*/
	inline YCTTickStruct* at(uint32_t idx)
	{
	...
	inline std::vector<YCTTickStruct>& getDataRef() { return m_ayTicks; }
	...
	*	追加一条Tick
	*/
	inline void	appendTick(const YCTTickStruct& ts)
	{
 ...
 */
class YCTTickSlice : public YCTObject
{
private:
	char			_code[MAX_INSTRUMENT_LENGTH];
	typedef std::pair<YCTTickStruct*, uint32_t> TickBlock;
	std::vector<TickBlock> _blocks;
	uint32_t		_count;

protected:
	YCTTickSlice() { _blocks.clear(); }
	inline int32_t		translateIdx(int32_t idx) const
	{
		if (idx < 0)
		{
...
public:
	static inline YCTTickSlice* create(const char* code, YCTTickStruct* ticks = NULL, uint32_t count = 0)
	{
		...
		YCTTickSlice* slice = new YCTTickSlice();
		memcpy(slice->_code, code, len);
		slice->_code[len] = 0;
		if(ticks != NULL)
		{
	...
	inline bool appendBlock(YCTTickStruct* ticks, uint32_t count)
	{
		if (ticks == NULL || count == 0)
			return false;
	...
	inline bool insertBlock(std::size_t idx, YCTTickStruct* ticks, uint32_t count)
	{
		if (ticks == NULL || count == 0)
			return false;
	...
	inline YCTTickStruct*	get_block_addr(std::size_t blkIdx)
	{
		if (blkIdx >= _blocks.size())
			return NULL;
	...
	inline const YCTTickStruct* at(int32_t idx)
	{
		if (_count == 0)
			return NULL;
 ...
 *	这样使用虽然更快,但是使用场景要非常小心,因为他依赖于基础数据对象
 */
class YCTOrdDtlSlice : public YCTObject
{
private:
	char				m_strCode[MAX_INSTRUMENT_LENGTH];
	YCTOrdDtlStruct*	m_ptrBegin;
	uint32_t			m_uCount;

protected:
	YCTOrdDtlSlice() :m_ptrBegin(NULL), m_uCount(0) {}
	inline int32_t		translateIdx(int32_t idx) const
	{
		if (idx < 0)
		{
...
public:
	static inline YCTOrdDtlSlice* create(const char* code, YCTOrdDtlStruct* firstItem, uint32_t count)
	{
		...
		YCTOrdDtlSlice* slice = new YCTOrdDtlSlice();
		memcpy(slice->m_strCode, code, len);
		slice->m_strCode[len] = 0;
	...
	inline const YCTOrdDtlStruct* at(int32_t idx)
	{
		if (m_ptrBegin == NULL)
			return NULL;
		idx = translateIdx(idx);
  ...
  *	这样使用虽然更快,但是使用场景要非常小心,因为他依赖于基础数据对象
  */
class YCTOrdQueSlice : public YCTObject
{
private:
	char				m_strCode[MAX_INSTRUMENT_LENGTH];
	YCTOrdQueStruct*	m_ptrBegin;
	uint32_t			m_uCount;

protected:
	YCTOrdQueSlice() :m_ptrBegin(NULL), m_uCount(0) {}
	inline int32_t		translateIdx(int32_t idx) const
	{
		if (idx < 0)
		{
...
public:
	static inline YCTOrdQueSlice* create(const char* code, YCTOrdQueStruct* firstItem, uint32_t count)
	{
		if (count == 0 || firstItem == NULL)
			return NULL;
		...
		YCTOrdQueSlice* slice = new YCTOrdQueSlice();
		memcpy(slice->m_strCode, code, len);
		slice->m_strCode[len] = 0;
		slice->m_ptrBegin = firstItem;
		slice->m_uCount = count;
	...
	inline const YCTOrdQueStruct* at(int32_t idx)
	{
		if (m_ptrBegin == NULL)
			return NULL;
		idx = translateIdx(idx);
  ...
  *	这样使用虽然更快,但是使用场景要非常小心,因为他依赖于基础数据对象
  */
class YCTTransSlice : public YCTObject
{
private:
	char			m_strCode[MAX_INSTRUMENT_LENGTH];
	YCTTransStruct*	m_ptrBegin;
	uint32_t		m_uCount;

protected:
	YCTTransSlice() :m_ptrBegin(NULL), m_uCount(0) {}
	inline int32_t		translateIdx(int32_t idx) const
	{
		if (idx < 0)
		{
...
public:
	static inline YCTTransSlice* create(const char* code, YCTTransStruct* firstItem, uint32_t count)
	{
		if (count == 0 || firstItem == NULL)
			return NULL;
		...
		YCTTransSlice* slice = new YCTTransSlice();
		memcpy(slice->m_strCode, code, len);
		slice->m_strCode[len] = 0;
		slice->m_ptrBegin = firstItem;
		slice->m_uCount = count;
	...
	inline const YCTTransStruct* at(int32_t idx)
	{
		if (m_ptrBegin == NULL)
			return NULL;
		idx = translateIdx(idx);
