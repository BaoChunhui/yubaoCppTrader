...
/*
 *	YCTVariant是一个通用数据容器,设计目标是Json的Value类
 *	和Json不同的地方在于,YCTVariant满足yubaoCppTrader系统内的派生关系
 *	可以通过引用计数管理数据,从而减少数据复制
 */
class YCTVariant : public YCTObject
{
public:
    typedef YCTArray					ChildrenArray;
    typedef YCTHashMap<std::string>		ChildrenMap;
...
private:
	...
	static inline YCTVariant* create(int32_t i32)
	{
		YCTVariant* ret = new YCTVariant();
		ret->_type = VT_Int32;
		char s[32] = { 0 };
		sprintf(s, "%d", i32);
	...
	static inline YCTVariant* create(uint32_t u32)
	{
		YCTVariant* ret = new YCTVariant();
		ret->_type = VT_Uint32;
		char s[32] = { 0 };
	...
	static inline YCTVariant* create(int64_t i64)
	{
		YCTVariant* ret = new YCTVariant();
		ret->_type = VT_Int64;
		char s[32] = { 0 };
	...
	static inline YCTVariant* create(uint64_t u64)
	{
		YCTVariant* ret = new YCTVariant();
		ret->_type = VT_Uint64;
		char s[32] = { 0 };
	...
	static inline YCTVariant* create(double _real)
	{
		YCTVariant* ret = new YCTVariant();
		ret->_type = VT_Real;
		char s[32] = { 0 };
	...
	static inline YCTVariant* create(const char* _string)
	{
		YCTVariant* ret = new YCTVariant();
		ret->_type = VT_String;
	...
	static inline YCTVariant* create(bool _bool)
	{
		YCTVariant* ret = new YCTVariant();
...
public:
    static inline YCTVariant* createObject()
    {
        YCTVariant* ret = new YCTVariant();
        ret->_type = VT_Object;
	...
	static inline YCTVariant* createArray()
	{
		YCTVariant* ret = new YCTVariant();
		ret->_type = VT_Array;
	...
	inline bool asBoolean() const
	{
		if (_value._string)
		{
			return yubao_stricmp(_value._string->c_str(), "true") == 0 || yubao_stricmp(_value._string->c_str(), "yes") == 0;
		}
	...
	inline YCTVariant* get(const char* name) const
	{
		if (_type != VT_Object)
			return NULL;
		...
		YCTVariant* ret = static_cast<YCTVariant*>(_value._map->get(name));
		return ret;
	}

	inline YCTVariant* get(const std::string& name) const
	{
		if (_type != VT_Object)
			return NULL;
	...
	inline YCTVariant* get(uint32_t idx) const
	{
		if (_type != VT_Array)
			return NULL;
		...
		YCTVariant* ret = static_cast<YCTVariant*>(_value._array->at(idx));
		return ret;
	}
	...
	inline bool append(const char* _name, const char* _string)
	{
		...
		YCTVariant* item = YCTVariant::create(_string);
	...
	inline bool append(const char* _name, int32_t _i32)
	{
		...
		YCTVariant* item = YCTVariant::create(_i32);
		_value._map->add(_name, item, false);
		//item->release();
	...
	inline bool append(const char* _name, uint32_t _u32)
	{
		...
		YCTVariant* item = YCTVariant::create(_u32);
		_value._map->add(_name, item, false);
		//item->release();
	...
	inline bool append(const char* _name, int64_t _i64)
	{
		...
		YCTVariant* item = YCTVariant::create(_i64);
		_value._map->add(_name, item, false);
		//item->release();
	...
	inline bool append(const char* _name, uint64_t _u64)
	{
		...
		YCTVariant* item = YCTVariant::create(_u64);
		_value._map->add(_name, item, false);
		//item->release();
	...
	inline bool append(const char* _name, double _real)
	{
		...
		YCTVariant* item = YCTVariant::create(_real);
		_value._map->add(_name, item, false);
		//item->release();
	...
	inline bool append(const char* _name, bool _bool)
	{
		...
		YCTVariant* item = YCTVariant::create(_bool);
		_value._map->add(_name, item, false);
		//item->release();
	...
	inline bool append(const char* _name, YCTVariant *item, bool bAutoRetain = true)
	{
	...
	inline bool append(const char* _str)
	{
		...
		YCTVariant* item = YCTVariant::create(_str);
	...
	inline bool append(YCTVariant *item, bool bAutoRetain = true)
	{
